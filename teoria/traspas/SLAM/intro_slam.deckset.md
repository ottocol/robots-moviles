...
- $$x_1,...x_t$$: posiciones del robot
- $$m$$: mapa, típicamente formado por *landmarks*
- $$z_1,...z_t$$, mediciones de los sensores
- $$u_1,...u_t$$, *movimientos* del robot
...
Las fórmulas van a ser prácticamente iguales, lo que cambia es que tenemos ahora la parte de los *landmarks*
...
Solo se actualiza la parte en la que intervienen las coordenadas del robot (ya que se supone que el mapa es estático). Gracias a esto la actualización se puede implementar con coste *lineal* con el número de *landmarks*.
...
- Cada **partícula** tiene un **mapa asociado** (una rejilla)
...
- Como una **partícula** supone una **posición** del robot, para actualizar el mapa podemos aplicar el algoritmo de **mapeado con posición conocida** que ya  visteis en el tema anterior
